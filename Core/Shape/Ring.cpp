...
namespace core {
    void Ring::UpdateData() {
        ...
        m_MeshData.IndexCount = m_RingIndices.size();
        m_MeshData.IndexData = m_RingIndices.data();
        m_MeshData.VertexData = m_RingVertexData.data();
        m_MeshData.VertexDataSize = m_RingVertexData.size() * sizeof(m_RingVertexData[0]);
    }
}
